...
### User Aggregate
- **Root**: USER
- **Components**: WALLET, CREDIT_ACCOUNT
- **Invariants**: User can have multiple wallets per currency, only one credit account

### Transaction Aggregate
- **Root**: TRANSACTION
- **Components**: LEDGER_ENTRY, PAYMENT_METADATA, INSTALLMENT_SCHEDULE
- **Invariants**: Transactions are immutable after creation, must have balanced ledger entries
...
- **Transactions**: Retained indefinitely (regulatory requirement)
- **Ledger Entries**: Retained indefinitely (regulatory requirement)
- **Wallets**: Soft delete only (status = 'CLOSED')
- **Idempotency Keys**: 24-hour TTL, purged daily
- **Payment Metadata**: Retained with transaction (indefinitely)
...
- Review [Database Schema](./database-schema.md) for complete DDL scripts
- See [Payment Types](./payment-types.md) for payment type details
- Explore [Ledger Model](./ledger-model.md) for double-entry bookkeeping
